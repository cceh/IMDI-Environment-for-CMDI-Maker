...
imdi_environment.workflow[2] = (function(){
	...
	my.handleImportFileInputChange = function (evt){
		...
		readFileAsText(file, function(result){
			...
			//before importing ANYTHING, check if all already existing actors
			//are named properly
			if (!isEveryActorNamed()){
			...
			//nowadays, actors are saved as an ObjectList.
			//so check, if var imported_actors is an ObjectList. if so, take only the array of objects from it
			if (imported_actors.info && imported_actors.info == "ObjectList state"){

				imported_actors = imported_actors.list;
				console.info("Imported file is an ObjectList of actors.");

			}

			for (var a=0; a<imported_actors.length; a++){

				my.createNewActor(imported_actors[a]);
	...
	my.createNewActor = function(actor_to_put){
		...
		//add actor to ObjectList and obtain the actor_id
		var actor_id = my.actors.add(actor_to_put);

		console.log('Yeah, actor inserted! actor_id = ' + actor_id);
