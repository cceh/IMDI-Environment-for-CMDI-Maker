...
imdi_environment.cmdi_generator = function(data, l){
	"use strict";
	...
	var insertWrittenResource = function(file){
		...
		xml.element("Derivation","Unspecified");
		...
		xml.open("Validation");
			xml.element("Type","Unspecified");
			xml.element("Methodology","Unspecified");
	...
	var createCorpus = function(corpus, sessions){
		...
		if (sessions.length > 0){
			...
			for (var i = 0; i < sessions.length; i++){
				...
				xml.open("ResourceProxy", [["id", IDREFS[i]]]);
				xml.element("ResourceType", "Metadata", [["mimetype", "application/x-cmdi+xml"]]);
